...
const CharacterCard: React.FC<CharacterCardProps> = ({ characterKey, content, isLast, onRegenerate, onEdit }) => {
  const char = CHARACTERS[characterKey] || CHARACTERS['NIJIKA'];
  const isNarrator = characterKey === 'NARRATOR';

  if (isNarrator) {
    return (
      <div className="group flex flex-col gap-2 mb-8 animate-fade-in last:mb-0 relative">
        <div className="flex items-center justify-center gap-4">
          ...
          {onEdit && (
            <button 
              onClick={onEdit}
              className="absolute right-0 opacity-0 group-hover:opacity-100 transition-opacity p-2 text-white/30 hover:text-white/60"
              title="Edit scene"
            >
